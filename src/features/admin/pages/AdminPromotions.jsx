...
const STORAGE_KEY = 'gympro_admin_promotions';
...
const seedPromotions = [
  {
    ...
    endDate: '2026-04-15',
    status: 'Active',
    description: 'Boost signups with a limited-time 25% membership discount.',
  },
  {
    id: 2,
    ...
    endDate: '2026-05-01',
    status: 'Draft',
    description: 'Flat discount for users coming from referral campaigns.',
  },
];
...
function AdminPromotions() {
  const [promotions, setPromotions] = useState(() => readPromotions());
  ...
  const handleDelete = (id) => {
    const existing = promotions.find((item) => item.id === id);
    const next = promotions.filter((item) => item.id !== id);
    persistPromotions(next);
    setToast({
      open: true,
      message: `${existing?.title || 'Promotion'} deleted successfully.`,
      severity: 'success',
    });
  };
  ...
  const handleSubmit = () => {
    ...
    const duplicate = promotions.find((item) => item.code === normalizedCode && item.id !== editingId);
    if (duplicate) {
    ...
    if (editingId) {
      const next = promotions.map((item) => {
    ...
    <MotionBox variants={containerVariants} initial="hidden" animate="visible" sx={{ pb: 2.4 }}>
      <MotionBox variants={itemVariants} mb={1.8}>
        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={1.2}>
          <Box>
            <Typography sx={{ fontWeight: 900, fontSize: { xs: '1.4rem', md: '1.65rem' } }}>
              Promotions Management
            </Typography>
            <Typography sx={{ color: 'text.secondary', mt: 0.35 }}>
              Create and manage ad promotions with modern campaign controls.
            </Typography>
          </Box>
          <Button
            variant="contained"
      ...
      <Grid container spacing={1.6}>
        {filteredPromotions.map((promotion) => (
          <Grid key={promotion.id} size={{ xs: 12, md: 6 }}>
            <MotionCard
              ...
              <CardContent sx={{ p: 2.2, '&:last-child': { pb: 2.2 } }}>
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1.2}>
                  ...
                  <Chip
                    ...
                    sx={{
                      fontWeight: 700,
                      backgroundColor: promotion.status === 'Active' ? 'rgba(16, 185, 129, 0.15)' : 'rgba(148, 163, 184, 0.16)',
                      color: promotion.status === 'Active' ? '#059669' : '#64748b',
                    }}
                  />
                </Stack>
                ...
                <Typography sx={{ color: 'text.secondary', fontSize: '0.82rem', mt: 1.2, minHeight: 42 }}>
        ...
        <MotionBox variants={itemVariants} sx={{ mt: 1.4 }}>
          <Card sx={{ borderRadius: 2.3, border: '1px dashed', borderColor: 'divider' }}>
            <CardContent>
              ...
              <Typography sx={{ color: 'text.secondary', fontSize: '0.86rem', mt: 0.4 }}>
                Try a different keyword or create a new promotion.
              </Typography>
            </CardContent>
          </Card>
        </MotionBox>
      ...
      <Dialog open={isDialogOpen} onClose={closeDialog} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          ...
          {formError && (
            <Alert severity="error" sx={{ mt: 1.4 }}>
              {formError}
            </Alert>
          )}
        </DialogContent>
